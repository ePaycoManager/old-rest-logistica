...
class CotizarController extends ApiController
{
	...
	public function Cotizar(Request $request, $operador = null)
    {
    	if($operador != null) {
	        if ($operador == 'tcc') {
	        	return $cotizacion = $this->generateResponse($this->tccCotizacion($this->getParameters($request), $request),'true','200','Respuesta de TCC dada correctamente');
		    } else {
    ...
    protected function tccCotizacion($params,Request $request){
	    ...
	    $consultarliquidacion->Liquidacion->idunidadestrategicanegocio = $params['idunidadestrategicanegocio'];// 2 siempre superpone unidad 1 - -  mensajeria solo cotiza de a 1 unidad
